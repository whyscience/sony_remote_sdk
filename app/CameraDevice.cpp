...
bool CameraDevice::get_lens_information()
{
    //std::this_thread::sleep_for(1s);
    load_properties();
    tout << "get_lens_information \n";
    std::vector < SCRSDK::CrLensInformation * > m_lensInfo;// Call the request
    SDK::CrError err = SCRSDK::RequestLensInformation(m_device_handle);
    tout << "RequestLensInformation err = "<< err <<'\n';
    if (CR_SUCCEEDED(err)){
        // When the OnWarning callback notifies you of success:
        CrInt32u numOfList = 0;
        SCRSDK::CrLensInformation *list = nullptr;
        err = SCRSDK::GetLensInformation(m_device_handle, &list, &numOfList);
        tout << "GetLensInformation err = "<< err <<'\n';
        if (CR_SUCCEEDED(err) && 0 < numOfList) {
            for (int i = 0; i < numOfList; ++i) {
                ...
                tout << "normalizedValue: " << item->normalizedValue << ", focusPosition: " << item->focusPosition <<'\n';
                m_lensInfo.push_back(item);
            }
            // release of list pointer
            SCRSDK::ReleaseLensInformation(m_device_handle, list);
            return true;
        }
    }
    return false;
...
void CameraDevice::load_properties(CrInt32u num, CrInt32u* codes)
{
    ...
    if (prop_list && nprop > 0) {
        ...
        for (std::int32_t i = 0; i < nprop; ++i) {
            ...
            int nval = 0;
            tout << "prop.GetCode() = "<< prop.GetCode() << ", i = " << i <<"\n";
            switch (prop.GetCode()) {
            ...
            case SDK::CrDevicePropertyCode::CrDeviceProperty_LensInformationEnableStatus:
                ...
                tout << "lens_information_enable_status.writable = "<< m_prop.lens_information_enable_status.writable <<'\n';
                tout << "lens_information_enable_status.current = "<< m_prop.lens_information_enable_status.current <<'\n';
                break;
